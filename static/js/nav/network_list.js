...
$("#del_network_button").click(function () {
    ...
    if (count > 0) {
        $.showConfirm("确定要删除吗?", delete_selected_network);
    }
});

// 删除选中的渠道
function delete_selected_network() {
    var content = '';
    $('#t_network_control > tbody  > tr').each(function () {
        var $check_box = $(this).find("td:eq(0)").find("input[name='network_list[]']");
        if ($check_box.prop('checked')) {
...
// 根据返回值从列表中删除用户信息
function delete_network_list(response) {
