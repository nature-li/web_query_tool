...
// 根据返回值从列表中删除用户信息
function delete_user_list(response) {
...
$("#del_user_button").click(function () {
    ...
    if (count > 0) {
        $.showConfirm("确定要删除吗?", delete_selected_user);
    }
});
...
$(document).on("click", ".user-edit-button", function () {
    ...
    edit_user(user_id, user_account, user_right);
});

// 根据response更新用户列表
function edit_user_page(response) {
    if (response.success == 'false') {
        $.showErr("更新失败");
        return;
    }
...
// 编辑用户信息
function edit_user(user_id, user_account, user_right) {
...
// 根据ajax返回值更新页面
function append_user_list(data) {
